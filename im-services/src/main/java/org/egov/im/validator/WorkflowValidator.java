...
import static org.egov.im.util.WorkflowConstants.CITIZEN_TYPE;
import static org.egov.im.util.WorkflowConstants.RATE_ACTION;
import static org.egov.im.util.WorkflowConstants.SENDBACKTOCITIZEN;
        ...
        for(ProcessStateAndAction processStateAndAction : processStateAndActions){
            ...
            if(action==null && !processStateAndAction.getCurrentState().getIsTerminateState())
                throw new CustomException("INVALID ACTION","Action not found for businessIds: "+
                        processStateAndAction.getCurrentState().getBusinessService().getUuid());
            ...
            Boolean isRoleAvailable = util.isRoleAvailable(roles,action.getRole());
    ...
    private List<String> getRolesFromState(State state){
        ...
        if(!CollectionUtils.isEmpty(state.getActions())){
            state.getActions().forEach(action -> {
                if(!action.getCurrentStatee().getUuid().equalsIgnoreCase(action.getNextState()))
                    transitionRoles.addAll(action.getRole());
            });
        }
