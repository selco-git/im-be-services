...
import static org.egov.im.repository.querybuilder.UserTypeQueryBuilder.SELECT_FAILED_ATTEMPTS_BY_USER_SQL;
import static org.egov.im.repository.querybuilder.UserTypeQueryBuilder.SELECT_NEXT_SEQUENCE_USER;
...
import org.egov.im.repository.querybuilder.UserTypeQueryBuilder;
import org.egov.im.repository.rowmapper.UserResultSetExtractor;
...
public class UserRepository {
     ...
     * @return
     */
    public boolean isUserPresent(String userName, String tenantId, String userType) {
    ...
    public void update(final User user, User oldUser, long userId, String uuid) {
        ...
        if (user.getPermanentAndCorrespondenceAddresses() != null) {
            addressRepository.update(user.getPermanentAndCorrespondenceAddresses(), user, user.getTenantId());
        }
    }
...
//		};
     ...
     */
    private List<Role> fetchRolesByCode(Set<String> roleCodes, String tenantId) {


        List<Role> validatedRoles = roleRepository.findRolesByCode(roleCodes, tenantId);
    ...
    private Map<String, Role> fetchRolesFromMdms(List<User> users) {
        ...
        List<Role> validatedRoles = fetchRolesByCode(roleCodes, users.get(0).getTenantId());
    ...
    private User save(User entityUser) {
        ...
        userInputs.put("type", entityUser.getType());
